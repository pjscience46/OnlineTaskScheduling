...
for name in name_list :
    ...
    for row in reader :
        if row[0] != "p" :
            ...
            Paper[index] += [float(row[1]) ]
            Min_time[index] += [float(row[2]) ]
            lower[index] += [float(row[3])]
