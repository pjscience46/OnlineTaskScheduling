...
min_beta = x.iloc[min_index]
min_mu = y.iloc[min_index]
min_value = z.iloc[min_index]

# Plot the red X marker
plt.scatter(min_beta, min_mu, color='red', s=150, marker='X',
...
if (min_beta - x_min) < x_margin:
    x_offset = 20  # move right
elif (x_max - min_beta) < x_margin:
...
plt.annotate(f"{min_value:.2f}", (min_beta, min_mu),
