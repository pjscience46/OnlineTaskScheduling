...
class Graph:
    ...
    def get_index_from_task(self,task):
        """Return the index of a certain task in the nodes list. We may not use this function because it appears that
         the native function index() already does that but I already coded it so let's keep it there"""
        nodes = self.get_nodes()
        for i in range(len(nodes)):
            if nodes[i] == task :
                return i
        print("ERROR : THE TASK IS NOT IN THE NODES LIST")
        return -1
