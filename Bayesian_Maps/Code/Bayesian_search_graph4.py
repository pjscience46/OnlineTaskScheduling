...
def plot_bo_surface_from_csv(
    ...
    if {best_p1_col, best_p2_col, best_y_col}.issubset(df.columns):
        ...
        best_source = "global_best_* columns (last row)"
    else:
    ...
    # Leave surface plain: NO contourf
    # Color the contour lines instead (blue=low, red=high)
    cs = plt.contour(P2g, P1g, MU, levels=35, cmap="RdBu_r", linewidths=1.2)
    plt.colorbar(cs, label="Surrogate mean μ(P1,P2) (objective)")

    # Observed points (x=P2, y=P1)
    plt.scatter(X[:, 1], X[:, 0], marker="x", c="black", s=45, linewidths=1.2, label="Evaluations")
    ...
    plt.xlabel("P2")
    plt.ylabel("P1")
    plt.title("Bayesian surrogate objective map (blue=low/better, red=high/worse)")
...
if __name__ == "__main__":
    ...
    CSV_PATH = r"C:\Users\m779p635\OneDrive - University of Kansas\Extension_Scheduling\OnlineTaskScheduling\FCFS\BO_MAST\combo_results_comm.csv"
